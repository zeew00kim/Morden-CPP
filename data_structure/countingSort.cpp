...
// 당장 필요한 일정 페이지만 메모리의 프레임에 적재하고 나머진 보조기억장치에 저장해두는데 
// 이 때 메모리의 프레임에 해당 페이지가 없기 때문에 디스크 블록에서 해당 데이터를 찾아야 한다.
// 디스크에서 데이터를 탐색하는 속도는 매우 느리고 이로 인해 전체 시스템 사용 효율이 저하되는
// 문제가 발생할 수 있다. 따라서, 말이 길었지만 공간복잡도 면에서 손실이 있을 수 있기 때문에
// 이런 경우에는 비교 기반 정렬 중 속도가 빠른 퀵/합병 정렬을 사용하는 것이 좋은 선택이다.